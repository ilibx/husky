...
function openAdd(parentId: number | null) {
  ...
  form.value = { name: '', path: '', icon: '', parent_id: parentId, sort: 0, roles: '', hidden: false }
  ...
  <div>
    ...
    <el-card>
      <div v-loading="loading">
        ...
        <el-table v-else :data="treeData" row-key="id" stripe border default-expand-all style="width:100%">
          ...
          <el-table-column prop="roles" label="可见角色" min-width="200">
            <template #default="{ row }">
              <el-tag v-if="!row.roles" size="small" type="success">全部</el-tag>
              <el-tag v-else v-for="r in row.roles.split(',')" :key="r" size="small" style="margin-right:4px">{{ r }}</el-tag>
            </template>
          </el-table-column>
          <el-table-column prop="hidden" label="隐藏" width="70">
            <template #default="{ row }">
              <el-tag v-if="row.hidden" type="warning">是</el-tag>
    ...
    <el-dialog v-model="dialogVisible" :title="editingId ? '编辑菜单' : '添加菜单'" width="550px">
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="路径">
          <el-input v-model="form.path" placeholder="/example（空=父菜单）" />
        </el-form-item>
